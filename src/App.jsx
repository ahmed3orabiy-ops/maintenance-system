...
export default function App() {
  ...
  const addExpense = (entry) => {
    const next = [...expenses, { ...entry, id: uid() }];
    saveExpenses(next);
  ...
  const addRevenue = (entry) => {
    ...
    saveRevenues([...revenues, { ...entry, total, id: uid() }]);
  ...
  const addFuelRecord = (rec) => {
    ...
    const total = (Number(rec.quantity) || 0) * (Number(rec.pricePerLiter) || 0);
  ...
  const addEquipmentCode = (c) => {
    saveEquipmentCodes([...equipmentCodes, { ...c, id: uid() }]);
    showToast("تم إضافة الكود");
  };
  const updateEquipmentCode = (id, updates) => {
    saveEquipmentCodes(equipmentCodes.map((c) => (c.id === id ? { ...c, ...updates } : c)));
  ...
  return (
    <div dir="rtl" className="min-h-screen flex" style={{ background: COLORS.cream, fontFamily: "'Cairo', sans-serif" }}>
      <style>{`
        ...
        * { font-family: 'Cairo', sans-serif; }
        ...
        @media print {
          ...
          #print-area table { width: 100%; font-size: 10.5px; }
      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        ...
        {loading ? (
          ...
          <div className="max-w-6xl mx-auto px-4 md:px-8 py-6 md:py-8">
            {view === "home" && <HomeView expenses={expenses} custodies={custodies} revenues={revenues} custodyTotals={custodyTotals} onNavigate={setView} />}
            ...
            {view === "entry" && <EntryForm custodies={custodies} expenses={expenses} onAdd={addExpense} onGoCustodies={() => setView("custodies")} />}
            {view === "revenue" && <RevenueView revenues={revenues} expenses={expenses} onAdd={addRevenue} onDelete={deleteRevenue} />}
            ...
            {view === "database" && <DatabaseView expenses={expenses} custodies={custodies} onDelete={deleteExpense} onUpdate={updateExpense} />}
            {view === "equipment" && <EquipmentView expenses={expenses} revenues={revenues} />}
            {view === "profitability" && <ProfitabilityView expenses={expenses} revenues={revenues} fuelRecords={fuelRecords} salaries={salaries} equipmentCodes={equipmentCodes} />}
            ...
            {view === "equipmentCodes" && <EquipmentCodesView codes={equipmentCodes} onAdd={addEquipmentCode} onUpdate={updateEquipmentCode} onDelete={deleteEquipmentCode} onImport={bulkImportEquipmentCodes} />}
...
/* ============================================================
   الصفحة الرئيسية
============================================================ */
function HomeView({ expenses, custodies, revenues, custodyTotals, onNavigate }) {
      ...
      <div className="rounded-2xl overflow-hidden relative" style={{ background: COLORS.navy }}>
        ...
        <div className="relative px-6 md:px-10 py-10 md:py-14 text-white">
          ...
          <div className="flex flex-wrap gap-6 md:gap-10">
            <div>
              <div className="text-xs text-white/50 font-semibold mb-1">إجمالي المصروفات</div>
              <div className="text-xl md:text-2xl font-extrabold tabular-nums">{fmtMoney(total)}</div>
            </div>
            <div>
...
/* ============================================================
   نموذج إدخال بند صرف
============================================================ */
function EntryForm({ custodies, expenses, onAdd, onGoCustodies }) {
  const empty = {
  ...
  const knownCodes = useMemo(() => {
    const map = {};
    expenses.forEach((e) => { if (e.equipmentCode) map[e.equipmentCode] = { type: e.equipmentType, brand: e.brand }; });
    return map;
  }, [expenses]);
  ...
  const handleCodeBlur = () => {
    const known = knownCodes[form.equipmentCode];
    if (known && (!form.equipmentType || !form.brand)) {
      setForm((f) => ({ ...f, equipmentType: f.equipmentType || known.type, brand: f.brand || known.brand }));
    }
  };
  ...
  const submit = (e) => {
    ...
    onAdd(form);
    setForm({ ...empty, source: form.source, custodyId: form.custodyId, category: form.category, date: form.date });
  };
        ...
        <SectionCard>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Field label="الجهة" required>
              <Select value={form.source} onChange={(e) => setForm((f) => ({ ...f, source: e.target.value, custodyId: "" }))}>
            ...
            <Field label="كود المعدة">
              <TextInput list="codes" value={form.equipmentCode} onChange={set("equipmentCode")} onBlur={handleCodeBlur} placeholder="مثال: EX-200-32" />
              <datalist id="codes">{Object.keys(knownCodes).map((c) => <option key={c} value={c} />)}</datalist>
            </Field>
            <Field label="النوع">
...
/* ============================================================
   قاعدة البيانات
============================================================ */
function DatabaseView({ expenses, custodies, onDelete, onUpdate }) {
  ...
  }, [expenses, q, catFilter, srcFilter, custodies]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4" style={{ background: "rgba(16,26,46,0.5)" }}>
          <form onSubmit={saveEdit} className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl p-6" style={{ background: COLORS.paper }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Field label="كود المعدة"><TextInput value={editing.equipmentCode} onChange={setField("equipmentCode")} /></Field>
      ...
      <SectionCard>
        ...
        {showFilters && (
          <div className="grid grid-cols-2 gap-3 mb-4 p-3 rounded-lg" style={{ background: COLORS.cream }}>
            <Field label="التصنيف">
        ...
        <>
          ...
          <div className="print-only-area" id="print-area">
            ...
            <table>
              ...
              <tfoot><tr><td colSpan={4}><b>الإجمالي</b></td><td><b>{fmtMoney(total)}</b></td></tr></tfoot>
            </table>
          </div>
        </>
      )}
...
function FuelView({ records, onAdd, onDelete, onImport }) {
  ...
  const empty = { date: todayISO(), code: "", vehicleType: "", odometerStart: "", odometerEnd: "", quantity: "", pricePerLiter: "", notes: "" };
  const [form, setForm] = useState(empty);
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }));

  const distancePreview = (Number(form.odometerEnd) || 0) - (Number(form.odometerStart) || 0);
  const totalPreview = (Number(form.quantity) || 0) * (Number(form.pricePerLiter) || 0);
  ...
  const rows = useMemo(() => {
    const term = q.trim().toLowerCase();
    return [...records]
      .filter((r) => !term || [r.code, r.vehicleType, r.notes].join(" ").toLowerCase().includes(term))
      .sort((a, b) => (b.date || "").localeCompare(a.date || ""));
  }, [records, q]);
  ...
  const handleFile = async (file) => {
    try {
      ...
      const idxQty = colIndex(["الكمية", "الكمية (لتر)"]);
      ...
      const idxNotes = colIndex(["ملاحظات", "المحطة"]);
      ...
      const parsed = rowsRaw.map((r) => {
        ...
        return {
          ...
          quantity,
          pricePerLiter: quantity ? total / quantity : 0,
          distance,
          total,
  ...
  const handleExport = () => {
    const rowsOut = records.map((r) => ({
      "التاريخ": r.date, "كود المعدة": r.code, "النوع": r.vehicleType,
    ...
    <div className="space-y-6">
        ...
        <SectionCard title={`السجل (${rows.length})`}>
          ...
          <div className="overflow-x-auto -mx-5">
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: COLORS.cream }}>
                  {["التاريخ", "كود المعدة", "النوع", "المسافة", "الكمية", "الإجمالي", "معدل الاستهلاك", ""].map((h) => (
                    <th key={h} className="px-3 py-2.5 text-right text-xs font-bold whitespace-nowrap" style={{ color: COLORS.slate }}>{h}</th>
                  ))}
                </tr>
              </thead>
      ...
      <div className="print-only-area" id="print-area">
        <div ref={printRef}>
          ...
          <table>
            <thead><tr>{["التاريخ", "كود المعدة", "النوع", "المسافة", "الكمية", "الإجمالي", "معدل الاستهلاك"].map((h) => <th key={h}>{h}</th>)}</tr></thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
            ...
            <tfoot><tr><td colSpan={4}><b>الإجمالي</b></td><td><b>{fmtNum(totalQty.toFixed(0))}</b></td><td><b>{fmtMoney(totalCost)}</b></td><td><b>{avgRate.toFixed(2)}</b></td></tr></tfoot>
          </table>
        </div>
      </div>
    </div>
...
/* ============================================================
   أكواد المعدات (مرجعي)
============================================================ */
function EquipmentCodesView({ codes, onAdd, onUpdate, onDelete, onImport }) {
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [showImport, setShowImport] = useState(false);
  const [importPreview, setImportPreview] = useState(null);
  ...
  const handleFile = async (file) => {
    try {
      ...
      const parsed = allRows.slice(1).filter((r) => r[idxCode]).map((r) => ({
        ...
        owner: idxOwner !== -1 && SOURCES.includes(r[idxOwner]) ? r[idxOwner] : SOURCES[0],
...
function ProfitabilityView({ expenses, revenues, fuelRecords, salaries, equipmentCodes }) {
  const ownerByCode = useMemo(() => {
    const map = {};
    equipmentCodes.forEach((c) => { map[normCode(c.code)] = c.owner; });
    expenses.forEach((e) => {
      const nc = normCode(e.equipmentCode);
      if (nc && !map[nc] && e.source) map[nc] = e.source;
    });
    return map;
  ...
  const rows = useMemo(() => {
    const realCodes = new Set(equipmentCodes.filter((c) => !isCostPoolCode(c.code)).map((c) => normCode(c.code)));
    expenses.forEach((e) => { const nc = normCode(e.equipmentCode); if (nc && !isCostPoolCode(nc)) realCodes.add(nc); });
    fuelRecords.forEach((r) => { const nc = normCode(r.code); if (nc) realCodes.add(nc); });
    revenues.forEach((r) => { const nc = normCode(r.equipmentCode); if (nc) realCodes.add(nc); });

    const directCost = {}, revenue = {};
    expenses.forEach((e) => {
      const nc = normCode(e.equipmentCode);
      if (!nc || isCostPoolCode(nc)) return;
      const t = (Number(e.cash) || 0) + (Number(e.transfer) || 0) + (Number(e.check) || 0);
      directCost[nc] = (directCost[nc] || 0) + t;
    });
    fuelRecords.forEach((r) => {
      const nc = normCode(r.code);
      if (!nc) return;
      directCost[nc] = (directCost[nc] || 0) + (Number(r.total) || 0);
    });
    revenues.forEach((r) => {
      const nc = normCode(r.equipmentCode);
      if (!nc) return;
      revenue[nc] = (revenue[nc] || 0) + (Number(r.total) || 0);
    });

    const poolByOwner = {}, salaryByOwner = {}, indirectByOwner = {}, totalDirectByOwner = {};
    SOURCES.forEach((s) => {
      ...
      indirectByOwner[s] = poolByOwner[s] + salaryByOwner[s];
      totalDirectByOwner[s] = 0;
    });
    realCodes.forEach((code) => {
      const owner = ownerByCode[code];
      if (owner && SOURCES.includes(owner)) totalDirectByOwner[owner] += directCost[code] || 0;
    });

    return [...realCodes].map((code) => {
      const owner = ownerByCode[code] || "غير محدد";
      const direct = directCost[code] || 0;
      const ownerDirectTotal = totalDirectByOwner[owner] || 0;
      const share = ownerDirectTotal > 0 && SOURCES.includes(owner) ? (indirectByOwner[owner] || 0) * (direct / ownerDirectTotal) : 0;
      const totalCost = direct + share;
      const rev = revenue[code] || 0;
      return { code, owner, direct, share, totalCost, revenue: rev, net: rev - totalCost };
    }).sort((a, b) => b.net - a.net);
  }, [expenses, revenues, fuelRecords, salaries, equipmentCodes, ownerByCode, poolCodeByOwner]);

  const totals = rows.reduce((acc, r) => ({
    direct: acc.direct + r.direct, share: acc.share + r.share, cost: acc.cost + r.totalCost,
    revenue: acc.revenue + r.revenue, net: acc.net + r.net,
  }), { direct: 0, share: 0, cost: 0, revenue: 0, net: 0 });
  ...
  return (
    <div className="space-y-6">
      <Header title="ربحية المعدات" sub="الإيراد ناقص التكلفة المباشرة (صيانة + سولار) ناقص نصيبها من التكلفة الموزّعة (مجمع التكلفة + المرتبات)" />

      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <KPICard label="إجمالي الإيراد" value={fmtMoney(totals.revenue)} icon={TrendingUp} />
        <KPICard label="التكلفة المباشرة" value={fmtMoney(totals.direct)} tone="gold" icon={Wrench} />
        <KPICard label="الحصة الموزّعة" value={fmtMoney(totals.share)} icon={Wallet} />
        <KPICard label="إجمالي التكلفة" value={fmtMoney(totals.cost)} tone="gold" icon={Wallet} />
        <KPICard label="صافي الربح الكلي" value={fmtMoney(totals.net)} icon={totals.net >= 0 ? TrendingUp : TrendingDown} />
      </div>

      <SectionCard title={`تفصيل الربحية لكل معدة (${rows.length})`}>
        <div className="overflow-x-auto -mx-5">
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: COLORS.cream }}>
                {["كود المعدة", "المالك", "التكلفة المباشرة", "الحصة الموزّعة", "إجمالي التكلفة", "الإيراد", "صافي الربح"].map((h) => (
                  <th key={h} className="px-3 py-2.5 text-right text-xs font-bold whitespace-nowrap" style={{ color: COLORS.slate }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.code} className="border-t" style={{ borderColor: COLORS.border }}>
                  <td className="px-3 py-2.5 font-semibold whitespace-nowrap">{r.code}</td>
                  <td className="px-3 py-2.5 text-xs whitespace-nowrap" style={{ color: COLORS.slate }}>{r.owner}</td>
                  <td className="px-3 py-2.5 tabular-nums whitespace-nowrap">{fmtMoney(r.direct)}</td>
                  <td className="px-3 py-2.5 tabular-nums whitespace-nowrap">{fmtMoney(r.share)}</td>
...
function RevenueView({ revenues, expenses, onAdd, onDelete }) {
  const [showForm, setShowForm] = useState(false);
  const empty = { equipmentCode: "", equipmentType: "", renter: "", startMonth: todayISO().slice(0, 7), months: "1", monthlyRate: "", paymentMethod: PAYMENT_METHODS[0], notes: "" };
  const [form, setForm] = useState(empty);
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }));

  const knownCodes = useMemo(() => {
    const map = {};
    expenses.forEach((e) => { if (e.equipmentCode) map[e.equipmentCode] = e.equipmentType; });
    return map;
  }, [expenses]);
  ...
  return (
    <div className="space-y-6">
          ...
          <SectionCard title="بيانات بند الإيراد الشهري">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Field label="كود المعدة" required>
                <TextInput list="rev-codes" value={form.equipmentCode} onChange={set("equipmentCode")}
                  onBlur={() => { const t = knownCodes[form.equipmentCode]; if (t && !form.equipmentType) setForm((f) => ({ ...f, equipmentType: t })); }}
      ...
      {revenues.length === 0 ? (
        ...
        <SectionCard title={`كل بنود الإيراد (${revenues.length})`}>
          <div className="overflow-x-auto -mx-5">
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: COLORS.cream }}>
              ...
              <tbody>
                {[...revenues].sort((a, b) => (b.startMonth || "").localeCompare(a.startMonth || "")).map((r) => (
                  <tr key={r.id} className="border-t" style={{ borderColor: COLORS.border }}>
          ...
          <table className="w-full border-collapse text-xs" style={{ minWidth: 900 }}>
            ...
            <thead>
              <tr style={{ background: COLORS.navy, color: "white" }}>
                {["م", "التاريخ", "كود المعدة", "النوع", "الماركة", "الموقع", "الغرض من الصرف", "نقدي", "تحويل", "شيك"].map((h) => (
            ...
            <tbody>
              {grouped.map((g) => (
                <React.Fragment key={g.cat}>
                  <tr style={{ background: COLORS.goldSoft }}>
                    <td colSpan={10} className="border px-3 py-1.5 font-extrabold" style={{ borderColor: COLORS.border, color: COLORS.navy }}>{g.cat}</td>
                  </tr>
                  {g.rows.map((e, i) => (
                    <tr key={e.id}>
              ...
              <tr style={{ background: COLORS.navy, color: "white" }}>
                <td colSpan={7} className="border px-3 py-2 font-extrabold text-center" style={{ borderColor: COLORS.navy }}>الإجمالي</td>
